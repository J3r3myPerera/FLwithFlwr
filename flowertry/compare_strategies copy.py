...
def run_fedavg(cfg: DictConfig, trainloaders, validationloaders, testloader, client_fn, initial_parameters, class_weights=None):
    """Run FedAvg strategy."""
    print("\n" + "=" * 60)
    ...
    strategy = fl.server.strategy.FedAvg(
        ...
        evaluate_fn=get_evaluate_fn(cfg.num_classes, testloader, class_weights),
...
def run_fedprox(cfg: DictConfig, trainloaders, validationloaders, testloader, client_fn, initial_parameters, class_weights=None):
...
def run_fedscaffold(cfg: DictConfig, trainloaders, validationloaders, testloader, initial_parameters, class_weights=None):
...
def main(cfg: DictConfig):
    ...
    trainloaders, validationloaders, testloader, class_weights = prepare_dataset(
    ...
    client_fn = generate_client_fn(trainloaders, validationloaders, cfg.num_classes, class_weights=class_weights)
    ...
    if 'fedavg' in strategies_to_run:
        try:
            history, elapsed_time = run_fedavg(cfg, trainloaders, validationloaders, testloader, client_fn, initial_parameters, class_weights)
    ...
    if 'fedprox' in strategies_to_run:
        try:
            result = run_fedprox(cfg, trainloaders, validationloaders, testloader, client_fn, initial_parameters, class_weights)
    ...
    if 'fedscaffold' in strategies_to_run:
        try:
            ...
            result = run_fedscaffold(cfg, trainloaders, validationloaders, testloader, initial_parameters, class_weights)
            history, elapsed_time = result[0], result[1]
    ...
    print("\n[Step 2/4] Training centralized model for comparison...")

    # Import model for predictions
    from model import Net, test
    from flwr.common import parameters_to_ndarrays

    # Train a centralized model to establish baseline performance
    print("\n  Training centralized model (100 epochs) on all training data...")
    print("  This establishes whether 60% is a fundamental limit or FL-specific issue")
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    centralized_model = Net(cfg.num_classes).to(device)
    
    # Gather all training data from clients
    all_train_data = []
    all_train_labels = []
    ...
    for epoch in range(100):
        ...
        for X, y in full_train_loader:
            ...
            optimizer.zero_grad()
            loss = criterion(centralized_model(X), y)
            loss.backward()
            optimizer.step()
    ...
    centralized_loss, centralized_accuracy = test(centralized_model, testloader, device, class_weights)
    
    print(f"\n  📈 Centralized Model Performance:")
    print(f"     Accuracy: {centralized_accuracy*100:.2f}%")
    print(f"     Loss: {centralized_loss:.4f}")
    
    # Compare with FL results
    print(f"\n  🔍 Performance Analysis:")
    if all_results:
        fl_accuracies = [r['final_accuracy'] for r in all_results if r['final_accuracy'] is not None]
        if fl_accuracies:
            best_fl_acc = max(fl_accuracies)
            avg_fl_acc = np.mean(fl_accuracies)
            print(f"     Best FL Accuracy: {best_fl_acc*100:.2f}%")
            print(f"     Avg FL Accuracy: {avg_fl_acc*100:.2f}%")
            
            # Diagnostic interpretation
            print(f"\n  💡 Diagnostic Interpretation:")
            if centralized_accuracy < 0.65:
                print(f"     🔴 FUNDAMENTAL LIMITATION DETECTED")
                print(f"     → Centralized model also caps at {centralized_accuracy*100:.1f}%")
                print(f"     → Issue is with DATA/MODEL, not federated learning")
                print(f"     → Recommended fixes:")
                print(f"       • Enable class weights (use_class_weights=true)")
                print(f"       • Add feature engineering (ratios, interactions)")
                print(f"       • Try wider/deeper model architecture")
                print(f"       • Check if task is inherently difficult")
            elif centralized_accuracy > 0.75:
                print(f"     🟡 FEDERATED LEARNING BOTTLENECK")
                print(f"     → Centralized: {centralized_accuracy*100:.1f}%, FL: {best_fl_acc*100:.1f}%")
                print(f"     → Gap: {(centralized_accuracy - best_fl_acc)*100:.1f}%")
                print(f"     → Issue is with FL configuration")
                print(f"     → Recommended fixes:")
                print(f"       • Reduce num_clients (more data per client)")
                print(f"       • Increase local_epochs (more local training)")
                print(f"       • Reduce batch_size (more gradient updates)")
                print(f"       • Increase num_rounds (more convergence time)")
            else:
                print(f"     🟢 REASONABLE FL PERFORMANCE")
                print(f"     → Centralized: {centralized_accuracy*100:.1f}%, FL: {best_fl_acc*100:.1f}%")
                print(f"     → Gap: {(centralized_accuracy - best_fl_acc)*100:.1f}%")
                print(f"     → FL overhead is acceptable")
    
    # Analyze prediction distribution
    print(f"\n[Step 3/4] Analyzing prediction distributions...")
    ...
    with torch.no_grad():
        for X, y in testloader:
            X = X.to(device)
            pred = centralized_model(X).argmax(dim=1)
            predictions.extend(pred.cpu().numpy())
            true_labels_list.extend(y.numpy())
    ...
    print("\n" + "=" * 60)
    print("CLASS BALANCE DIAGNOSTIC (Centralized Model)")
    print("=" * 60)
    
    class_names = ['Low (<7%)', 'Medium (7-12%)', 'High (>12%)']
    
    print("\n📊 True Label Distribution (Test Set):")
    true_counts = np.bincount(true_labels_array, minlength=cfg.num_classes)
    for cls, count in enumerate(true_counts):
        pct = 100 * count / len(true_labels_array)
        print(f"  Class {cls} ({class_names[cls]}): {count} samples ({pct:.1f}%)")
    
    print("\n🎯 Centralized Model Prediction Distribution:")
    ...
    for cls in range(cfg.num_classes):
        ...
        if cls_mask.sum() > 0:
            cls_acc = (predictions[cls_mask] == true_labels_array[cls_mask]).sum() / cls_mask.sum()
    ...
    if all_results:
        fl_accuracies = [r['final_accuracy'] for r in all_results if r['final_accuracy'] is not None]
        if fl_accuracies:
            print(f"  • Best FL accuracy: {max(fl_accuracies)*100:.2f}%")
            print(f"  • Performance gap: {(centralized_accuracy - max(fl_accuracies))*100:.2f}%")
    print(f"  • Class imbalance ratio: {true_dist['imbalance_ratio']:.2f}x")
    print(f"  • Prediction uniformity: {pred_uniformity:.2f}")

    if true_dist['imbalance_ratio'] > 3.0:
        print(f"\n  ⚠️  SEVERE CLASS IMBALANCE DETECTED")
        print(f"  • This is likely limiting model performance to majority class baseline")
        print(f"  • Recommendation: Enable class weights (use_class_weights=true)")
